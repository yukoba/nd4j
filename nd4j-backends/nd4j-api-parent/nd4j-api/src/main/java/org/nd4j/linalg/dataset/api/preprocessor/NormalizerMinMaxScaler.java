    ...
    */
    ...
    public void fit(DataSet dataSet) {
        ...
        if (maxMinusMin.min(1) == Nd4j.scalar(Nd4j.EPS_THRESHOLD)) {
            logger.info("API_INFO: Max minus min found to be zero. Transform will round upto epsilon to avoid nans.");
        }
    }
    ...
    public void preProcess(DataSet toPreProcess) {
        ...
        if (maxRange - minRange <= Nd4j.EPS_THRESHOLD) throw new RuntimeException("API_USE_ERROR: The given max value minus min value has to be greater than 0");
