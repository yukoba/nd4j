...
* In IntelliJ, create a brand-new project. (We'll call this example "AppTest".) 
...
* Your **entire** POM file for AppTest should look something like this:
...
* Now click on IntelliJ's *Run* menu and select *Edit Configurations*. This will open a window with a + sign on the upper lefthand side. 
![Alt text](../img/benchmarks_1.png)
* Click on the plus sign, and select *Application* from the drop-down menu. 
* Next, enter *org.nd4j.linalg.benchmark.app.BenchmarkRunnerApp* in the "Main Class" field. In the "Program Arguments" field, enter the terms *-n 5*. 
* On the bottom right of the same window, first click the "Apply" button, and then "OK" or "Run".
...
The numbers reported will be specific to your machine, the backend you choose, and the "Program Arguments" you enter. It's good to start with 5 iterations to test that benchmarks are working, and then increase them. 

All operations are dealing with a matrix of 10,000 rows x 10,000 columns, which flattens to a 100-million length vector. That's not an unrealistic number for real problems, so only matrices of this size or larger will give us meaningful tests. 
